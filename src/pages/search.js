...
export default function Search() {
  ...
  const searchData = async () => {
    ...
    try {
      const results = await axios('https://6389fd1dc5356b25a20e871a.mockapi.io/framework?search=' + name);
      setData(results.data);
    } catch (err) {
      setError(true);
      setTimeout(() => setError(false), 4000);
      ...
      <div className='max-w-5xl mx-auto w-full h-tampilan'>
        ...
        {error ? (
          ...
          <div className='grid grid-cols-1 gap-5 p-6 sm:grid-cols-2 md:grid-cols-3'>
            {data.length > 0 ? data.map(item => <CardList key={item.id} data={item} button='true' />) : <h2 className='font-bold text-xl text-center col-span-3'>Data not found</h2>}
          </div>
        )}
      </div>
    </Layout>
